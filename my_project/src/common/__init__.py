...
from common.csv_data import CSVEncoder
import re
from common.struct_type import AdapterAllToolStructType
...
# 清洗和解析来自大语言模型（LLM）的原始响应，将其转换为符合 kor_schema 定义的结构化数据
def paser_response_data(
        response,
        kor_schema: Object
        ):
    
    encoder = CSVEncoder(node=kor_schema)  # 将 Kor Schema 转换为 CSV 格式的解析规则（用于后续文本匹配）
    parser = KorParser(encoder=encoder, schema_=kor_schema)  # 核心解析器，根据 Schema 从文本中提取结构化数据
