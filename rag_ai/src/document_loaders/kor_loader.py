...
'''
一个基于 Kor (结构化数据抽取框架）的工具类，专门用于构建不同场景下的信息抽取链。
核心功能是通过预定义的结构化模板(Schema),指导大语言模型从文本中提取特定格式的数据。
'''
class KorLoader:
    @classmethod
    def form_kor_dreams_guidance_builder(    # 心理咨询对话的步骤引导
        ...
        schema = Object(
            id="script",
            # description="开放性引导问题",
            description="""开放性引导问题必须严格按以下规则生成：
        1. step_advice、step_description 的内容必须为非空且完整的
        2. step_advice 必须为合理的引导建议
        3. step_description 必须为咨询步骤的描述
        4. 输出格式必须完全匹配示例""",
            attributes=[
                Text(   # 具体引导话术
                    id="step_advice",
...
通过这种方式，您可以引导来访者自由表达他们的情感和思维，帮助他们更好地理解自己和他们与他人的互动。同时，保持开放和倾听，以便在需要时提供支持和建议。""",
    ...
    def form_kor_dreams_personality_builder(
        ...
        schema = Object(
            ...
            # description="性格信息(personality,subj)", 这是原来的，但是模型生成不稳定
            description="""性格信息(personality,subj)必须严格按以下规则生成：
        1. personality、subj 必须为非空且完整的
        2、personality 必须为合理的性格评价
        3. 输出格式必须完全匹配示例""",
            attributes=[
                Text(
                    id="personality",
...
综上所述，张毛峰和露ᥫᩣ在对话中表现出的是一种现代社交网络下成熟的交流方式，他们性格中包含了幽默感、社交技巧、适应性和直接性等特征。这些特点使他们能够在数字媒介主导的社交环境中游刃有余。
    ...
    def form_kor_dreams_task_step_builder(
        ...
        schema = Object(
            id="script",
            # description="Adapted from the novel into script,",  这是原来的，但是模型生成不稳定
            description="""必须严格按以下规则生成：
1. task_step_name、task_step_description 必须为非空且完整的
2、task_step_level 必须为非空且不重复的层级编号，严格按层次递增（如 '0', '0>1', '1>2'）
3. 禁止使用占位符如0、空值或无意义文本
4. 输出格式必须完全匹配示例
5. 严格控制步骤数量，最多只生成5个步骤""",
            attributes=[
                Text(
                    id="task_step_name",
