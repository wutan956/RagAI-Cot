...
from engine.entity.task_step.task_step import TaskStepNode
from engine.generate.code_generate import (
    CodeGenerator
)
from engine.storage.task_step_store.types import BaseTaskStepStore, RefTaskStepInfo
from engine.storage.task_step_store.utils import json_to_task_step, task_step_to_json

from engine.storage.kvstore.types import BaseKVStore
...
class KVTaskStepStore(BaseTaskStepStore):
    ...
    def __init__(
        ...
        self._namespace = namespace or DEFAULT_NAMESPACE  # namespace是空的，所以_namespace是字符串“task_step_store"
        self._node_collection = f"{self._namespace}/data"  # 就相当于在当前任务的节点中多了这三个，分别是"task_step_store/data,task_step_store/ref_task_step_info,task_step_store/metadata"
